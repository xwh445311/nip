                ...
                <div class="col-md-12">
                    ...
                    <div class="panel panel-primary">
                        ...
                        <div class="panel-body" style="padding: 5px;">
							<form action="<%=basePath%>/office/message/updateTsMessageInfo.do" id="tsMessageInfoForm" name="tsMessageInfoForm" method="post" role="form" enctype="multipart/form-data">
								<div class="alert alert-block alert-danger fade in errorContent" id="errContent" style="display: none"></div><br/>

                                <input type="hidden" name="miCode" id="miCode" value="${tsMessageInfo.miCode}"/>
								<div class="form-group">
									...
									<select class="form-control" id="miIssueDept" name="miIssueDept">
										...
										<option value="${item.key}" <c:if test="${item.key == tsMessageInfo.miIssueDept}">selected</c:if>>${item.value}</option>
										</c:forEach>
									</select>
								</div>
								...
								<div class="form-group">
									...
									<select class="form-control" id="miType" name="miType">
										<option value="">请选择</option>
										<option value="01" <c:if test="${'01' == tsMessageInfo.miType}">selected</c:if>>内部</option>
										<option value="02" <c:if test="${'02' == tsMessageInfo.miType}">selected</c:if>>外部</option>
									</select>
								</div>

								<div class="form-group">
									<label for="miLvl"><span class="text-danger ">*</span>通知级别</label>
									<efs:selParam name="miLvl" id="miLvl" type="TS_MESSAGE_LVL" cls="form-control" selVal="${tsMessageInfo.miLvl}"/>
								</div>

								<div class="form-group">
									<label for="miTitle"><span class="text-danger ">*</span>标题</label>
									<input type="text" class="form-control" id="miTitle" name="miTitle" placeholder="标题" value="${tsMessageInfo.miTitle}">
								</div>
								...
								<div class="form-group">
									...
									&nbsp;
									<input type="checkbox" value="Y" name="delFile" id="delFile">删除原附件
								</div>
