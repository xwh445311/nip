    ...
    <div class="main-content" >
        ...
        <div class="wrapper">
                        ...
                        <div class="panel-body" style="padding: 5px;">
                            <form class="form-inline" role="form">
                                <div class="form-group">
									批次：
									<select name="tiBatchId" id="tiBatchIdQuery" class="form-control">
										<option value="">请选择</option>
										<c:forEach items="${batchIds}" var="item">
										<option value="${item}" <c:if test="${sessionScope.eduTeaplanInfoForm.tiBatchId==item}">selected</c:if>  >${item}</option>
										</c:forEach>
									</select>
									&nbsp;
									学期：
									...
									<select name="tiStatus" id="tiStatusQuery" class="form-control">
										<option value="">请选择</option>
										<option value="2" <c:if test="${sessionScope.eduTeaplanInfoForm.tiStatus=='2'}">selected</c:if>>未评定</option>
			...
			<div class="row">
				<div class="col-md-12" id="gridPanel">
					<table  id="dataList"
                            ...
                            singleSelect="true"
                            collapsible="true"
                            url="<%=basePath %>/edu/teaplan/listEvalEduTeaplanInfo.do"
                            method="post">
						<thead>
							<tr>
								...
								<th data-options="field:'tiStatus',formatter:formatStatus" width="50">评定状态</th>
			...
			<div id="toolbar">
                ...
                <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-add" plain="true" onclick="newButton()">教案评定</a>
...
{
	if(val == '2'){
	    return "<font color='red'>未评定</font>";
    }else if(val == '3'){
        return "<font color='green'>已评定</font>";
    }
}
...
function newButton() {
    ...
    if(!row)
    {
        ShowErrorDialog("请选择要评价的教案");
        return;
    }

    //可以进行提交
    window.location.href="<%=basePath%>/edu/teaplan/gotoDoEvalTeaplan.do?tiDbId=" + row.tiDbId;
}
